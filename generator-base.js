...
Generator.prototype.getTemplate = function getTemplate(templateName) {
	...
	return (function (fname) {
		return this.readFileAsString(path.join(this.dev.templates, fname));
...
Generator.prototype.promptForModuleValues = function promptForModuleValues(extraPrompts, callback) {
	...
	var pathMsg = function () {return "(Path) " + this.dir.scripts;}.bind(this);
...
Generator.prototype.createModule = function createModule(values) {
	...
	if(!this.options["skip-add"])
		this.pushToConfig("scripts", module.name, path.join(this.dir.scripts, module.path, module.name) + '.js');

	module.path = path.join(this.dir.scripts, module.path, module.name);
...
Generator.prototype.wireScriptsToFile = function wireScriptsToFile(file, scripts) {
	...
	for(i in scripts) {
		scripts[i] = path.relative(this.dir.public, scripts[i]);
	}
